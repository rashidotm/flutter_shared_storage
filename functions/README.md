...
The package now uses a flat layout: originals live directly under `STORAGE_PREFIX`, and the function's outputs go in sibling `thumbs/` and `previews/` subfolders:

```
{STORAGE_PREFIX}/{nodeId}.{ext}          ← original (triggers the function)
...
Storage `onObjectFinalized` for anything under `${STORAGE_PREFIX}/`. The function:

1. Skips objects outside the configured `STORAGE_PREFIX`.
2. Skips its own outputs by ignoring anything with an additional `/` after the prefix (i.e., only processes files DIRECTLY under the prefix — objects in `thumbs/` or `previews/` are skipped).
3. Extracts `nodeId` from the filename (strip extension).
4. Downloads the original.
5. Resizes images with `sharp`, or extracts a video frame with `fluent-ffmpeg`.
6. Uploads `thumbs/{nodeId}.jpg` (256 px) and `previews/{nodeId}.jpg` (1024 px).
7. Updates `${FIRESTORE_PREFIX}/{nodeId}` with `thumbnailUrl` + `previewUrl`.
