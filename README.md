...
- [`functions/`](functions) — Cloud Functions that generate image/video thumbnails server-side on upload.
...
- **Server-side thumbnails.** Generated by a Cloud Function on Storage finalize, configured with `STORAGE_PREFIX` + `FIRESTORE_PREFIX` env vars matching the package's paths.
